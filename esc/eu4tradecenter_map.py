...
def main():
    ...
    trade_modifiers = {}
    for _, tree in parser.parse_files('common/event_modifiers/*'):
        for n, v in tree:
            trade_power = sum(v2.val for n2, v2 in v
                              if n2.val == 'province_trade_power_value')
            if trade_power:
                trade_modifiers[n.val] = trade_power
            ...
            if n.val == 'add_permanent_province_modifier':
                ...
                for n2, v2 in v:
                    if n2.val == 'add_permanent_province_modifier':
                        modifiers.append(v2['name'].val)
        trade_power = sum(trade_modifiers.get(x, 0) for x in modifiers)
        province_value[number] = trade_power
    ...
    for number, value in sorted(province_value.items()):
        if value == 0:
            continue
    ...
    out_path = rootpath / (mod + 'eu4tradecenter_map.png')
